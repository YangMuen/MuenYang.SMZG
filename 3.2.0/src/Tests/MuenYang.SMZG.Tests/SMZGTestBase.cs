...
using MuenYang.SMZG.Migrations.SeedData;
...
namespace MuenYang.SMZG.Tests
{
    public abstract class SMZGTestBase : AbpIntegratedTestBase<SMZGTestModule>
    {
        ...
        protected void UsingDbContext(Action<SMZGDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SMZGDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SMZGDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SMZGDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SMZGDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SMZGDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SMZGDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SMZGDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SMZGDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SMZGDbContext, Task<T>> func)
        {
            T result;
